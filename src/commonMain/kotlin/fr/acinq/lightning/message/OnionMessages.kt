...
object OnionMessages {
    ...
    sealed interface Destination {
        ...
        data class BlindedPath(val route: RouteBlinding.BlindedRoute) : Destination {
            override val introductionNodeId = route.introductionNodeId
        }
        ...
        data class Recipient(val nodeId: PublicKey, val pathId: ByteVector?, val padding: ByteVector? = null, val customTlvs: Set<GenericTlv> = setOf()) : Destination {
            override val introductionNodeId = NodeId.Standard(nodeId)
        }
    }
    ...
    private fun buildIntermediatePayloads(
        ...
        return if (intermediateNodes.isEmpty()) {
            ...
            intermediateNodes.drop(1).map { node -> setOf(RouteBlindingEncryptedDataTlv.OutgoingNodeId(NodeId.Standard(node.nodeId))) }
    ...
    fun buildRoute(
        ...
        val intermediatePayloads = buildIntermediatePayloads(
            intermediateNodes,
            setOf(RouteBlindingEncryptedDataTlv.OutgoingNodeId(NodeId.Standard(recipient.nodeId)))
        )
    ...
    private fun buildRouteFrom(
        ...
        when (destination) {
            ...
            is Destination.BlindedPath ->
                when {
                    destination.route.introductionNodeId == NodeId.Standard(originKey.publicKey()) ->
                        return try {
